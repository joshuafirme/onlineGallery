@extends('layouts.demo-public-gallerybase')

@section('title')
    Make It Memories | Demo Public Gallery
@endsection
    ...
    <div id="carouselContainer">
        <div id="carouselExampleControls" pause="false" class="carousel slide carousel-fade" data-bs-ride="carousel">
            <div class="carousel-inner">
                ...
                @endphp
                @foreach ($photosOnly as $key => $singleMedia)
                    @php
                    ...
                    <div class="carousel-item {{ $carouselClass }}" id="carousel-item">
                        <div class="carousel-item-content" id="carousel-item-content"
                            ...
                            <div class="d-flex align-items-center justify-content-center h-100 w-100"
                                ...
                                {{-- Image file --}}
                                <img src="{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}"
                                    class="d-block img-fluid" alt="Slide {{ $activeMediaId + 1 }}"
                                    style="object-fit: cover; max-width: 100%; max-height: 100%;" id="fullscreenImage">
                    ...
                    <div class="gallery-container">
                        ...
                        <div class="gallery-item">
                            @forelse ($media as $singleMedia)
                                <div class="wrapper" onmouseover="showDeleteButton(this)"
                                    onmouseout="hideDeleteButton(this)">
                                    <!-- Show delete button initially as hidden -->
                                    @if (Auth::user())
                                        <form id="deleteForm" method="POST"
                                            action="{{ route('delete-publicmedia', ['id' => $singleMedia->id]) }}"
                                    ...
                                    @endif
                                    <a href="{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}"
                                        download="{{ $singleMedia->file_path }}" class="download-link "
                                        style="z-index: 100;">
                                    ...
                                    @if (strpos($singleMedia->file_path, '.mp4') !== false || strpos($singleMedia->file_path, '.webm') !== false)
                                        ...
                                        <video controls>
                                            <source
                                                src="{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}"
                                                type="video/mp4">
                                            Your browser does not support the video tag.
                                        </video>

                                        <div class="view-link"
                                            onclick="openVideoModal('{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}')">
                                            view
                                        </div>
                                    @else
                                        <div class="view-link"
                                            onclick="openImageModal('{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}')">
                                            view
                                        </div>
                                        {{-- Image file --}}
                                        <img src="{{ asset('uploadedFiles/PublicMedia/' . $singleMedia->file_path) }}"
